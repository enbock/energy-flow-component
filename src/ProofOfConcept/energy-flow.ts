...
class EnergyFlow extends HTMLElement {
    ...
    private numParticles: number = 5000;
...
// Canvas-Größe anpassen (z.B. an die tatsächliche Größe des Elements)
        ...
        this.updateTimer = <any>setInterval(() => this.parseConnections(), 1000);
    }
...
// Erzeugt die Anfangszustände der Teilchen
    private initParticles(): void {
        ...
        for (let i = 0; i < this.numParticles; i++) setTimeout(() => {
            this.particles.push({
...
// Haupt-Animationsloop
        ...
        this.particles.forEach((p) => {
            ...
            if (p.target !== undefined) {
                ...
                if (sourceConn && p.progress < 1) {
                    const distSourceCenter = this.calculateDistance(
                        this.canvas.width / 2 + sourceConn.x,
                        this.canvas.height / 2 + sourceConn.y,
                        this.canvas.width / 2,
                    ...
                    if (p.progress < p.progressBefore) p.progress = 1;
                }
            ...
            const isInTargetZone = p.target !== undefined && targetConn &&
                ...
                ) < Math.sqrt(this.canvas.width * this.canvas.height) * (this.particleSize / 150) * this.targetSize * 4;
    ...
    private drawParticles() {
        ...
        ctx.save();
        this.particles.forEach((p) => {        // Erstelle einen radialen Farbverlauf
